...
public class GetCarDetailsFromBookingController {
    ...
    @org.springframework.web.bind.annotation.GetMapping("/api/v1/booking/{bookingId}/car")
    public org.springframework.http.ResponseEntity<com.example.demo.Entity.CarMaster> getCar(
            @org.springframework.web.bind.annotation.PathVariable long bookingId) {
        com.example.demo.Entity.CarMaster car = getDetailsOfCarFromBookingService.getCarDetailsByBookingId(bookingId);
        if (car != null) {
            return org.springframework.http.ResponseEntity.ok(car);
        }
